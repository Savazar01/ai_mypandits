...
// PATCH /api/v1/events/[id]/invitations/[invId]
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try {
    const backendRes = await fetch(
      `${PYTHON_BACKEND_URL}/api/v1/events/${id}/invitations/${invId}`,
      {
        method: "PATCH",
    ...
    console.error("[invitations/[invId] proxy] PATCH failed:", err?.message ?? err);
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    console.error("[invitations/[invId] proxy] DELETE failed:", err?.message ?? err);
    return NextResponse.json({ error: "Could not reach AI backend" }, { status: 503 });
  }
}
